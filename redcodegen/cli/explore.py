"""TUI explorer for browsing rcg generate output files."""
...
class SavePairScreen(ModalScreen[str | None]):
    ...
    def __init__(self, default_dir: str, code: str, tests: str, solution_file: str = "solution.py", test_file: str = "test_solution.py") -> None:
        super().__init__()
...
class ExploreApp(App[None]):
    """TUI for exploring rcg generate output."""
    ...
    def action_save_pair(self) -> None:
        ...
        self.push_screen(
            SavePairScreen(default_dir, rollout.code, scenario.tests, solution_file, test_file),
            on_dismiss,
        )
...
def explore(
    ...
    """Interactively explore rcg generate output."""
    if not path.exists():
